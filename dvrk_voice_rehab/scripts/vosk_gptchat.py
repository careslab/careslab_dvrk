...
#####################################################################
    #prompt type 0 is short, prompt type 1 is long
    temp = 1.0  
    ...
    if (prompt_type):
        completions = openai.ChatCompletion.create(
            ...
            messages=[
                ...
                {"role": "user", "content": "Given a phrase like this: 'Please Track the right tool' \
                    return the letters correspoing to the right answer: \
                    'TR': 'track the right tool must contain track or similar word', \
                    'TL': 'track the left tool  must contain track or similar word', \
                    'TM': 'track the middle of the tools',\
                    'ST': 'start or start moving camera', \
                    'SX': 'stop camera or stop everything', \
                    'KL': 'keep left or keep the left tool point in view',\
                    'KR': 'keep right keep the right tool point in view', \
                    'FT': 'indication to find my tools or lost tools', \
                    'TP': 'indication to take a picture or take picture',\
                    'SV': 'indication to start a video recording', \
                    'XV': 'indication to stop the video recording', \
                    'NV': 'something not valid or not understood"
                },
                {"role": "assistant", "content": "TR"},

                {"role": "user", "content": "Start recording the video"},
                {"role": "assistant", "content":  "SV"},

                {"role": "user", "content": "'Please take a picture '"}, 
        ...
        print ("prompt_type must be 0 or 1")
        return (choices ("NV"))
    ...
    if (index in listofpossiblecommands) and len(index) == 2:
        print ("====> temp= " + str(temp) + "index =" + str(index))
...
try:
    ...
    dump_time = open("./timings_query.txt","w")
    if args.filename:
            ...
            while True:
                ...
                if rec.AcceptWaveform(data):
                    ...
                    if ((res['partial'] == "") and (prompt == "") and (prevprompt != "")) :
                        ...
                        if (first):
                            start = timer()
                            cmd = AskGPT (prevprompt, long_message)
                            
                            #print ("long message")
                        else:
                            cmd = AskGPT (prevprompt, short_message)
                            #print ("short message")
