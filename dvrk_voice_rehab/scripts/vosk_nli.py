...
# candidate_labels =  ["davinci start","davinci stop","track right","track left","track middle",
#                     "keep left","keep right","keep middle","find my tools","take picture",
#                     "start video","stop video","start moving camera","stop moving camera",
#                     "stop camera","stop everything"]
candidate_labels = ["davinci start","davinci stop","davinci track right","davinci track left","davinci track middle",
                    "davinci keep left","davinci keep right","davinci keep middle","davinci find my tools",
                    "davinci take picture","davinci start video","davinci stop video","start moving camera",
                    "stop moving camera","stop camera","stop everything"]
            ...
            while True:
                ...
                if rec.AcceptWaveform(data):
                    ...
                    if ((res['partial'] == "") and (prompt == "") and (prevprompt != "")) :
                        print (prevprompt)
                        # print (prompt)
                        # output0 = classifier(prevprompt,candidate_labels=["davinci","da vinci"])
                        # output = {}
                        # if(output0['scores'][0]>0.6 and output0['scores'][1]>0.6):
                        #  hypothesis_template = "This sentence is similar to {}"
                        # output = classifier(prevprompt, candidate_labels)
                        # print(output)
                        # cmd = output['labels'][0]
                        cmd = "Something not valid"
                    else:
                        cmd = "Something not valid"
                        # print(output)
                    ...
                    #execute the command
                    '''
                    ["davinci start","davinci stop","davinci track right","davinci track left","davinci track middle",
                    "davinci keep left","davinci keep right","davinci keep middle","davinci find my tools","davinci take picture","davinci start video","davinci stop video","start moving camera","stop moving camera",
                    "stop camera","stop everything"]
                    '''
