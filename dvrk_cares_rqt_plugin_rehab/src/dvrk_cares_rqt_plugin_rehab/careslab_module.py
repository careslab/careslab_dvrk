...
class MyPlugin(Plugin):
    ...
    def __init__(self, context):
        ...
        ui_file = os.path.join(rospkg.RosPack().get_path('dvrk_cares_rqt_plugin_rehab'), 'resource', 'MyPlugin_Rehab.ui')
        # Extend the widget with all attributes and children from UI file
        loadUi(ui_file, self._widget)
        # Give QObjects reasonable names
        self._widget.setObjectName('MyPlugin')
        ...
        self._widget.strokeRehabRadioButton.pressed.connect(self._on_strokeRehabRadioButton_pressed)
        self._widget.leftArmRadioButton.pressed.connect(self._on_leftArmRadioButton_pressed)
        self._widget.rightArmRadioButton.pressed.connect(self._on_rightArmRadioButton_pressed)
    ...
    def _on_powerOnButton_pressed(self):
        ...
        self._widget.strokeRehabRadioButton.setEnabled(True)
    ...
    def _on_resetButton_pressed(self):
        ...
        print("RAN RESET")
        #rospy.Publisher('/rehab/right/run', Bool, latch=True, queue_size=1).publish(Bool(False))
        #rospy.Publisher('/rehab/left/run', Bool, latch=True, queue_size=1).publish(Bool(False))
    ...
    def _on_strokeRehabRadioButton_pressed(self):
        self._widget.leftArmRadioButton.setEnabled(True)
        self._widget.rightArmRadioButton.setEnabled(True)

    def _on_leftArmRadioButton_pressed(self):
        print("Start rehab - left arm")
        rospy.Publisher('/rehab/left/run', Bool, latch=True, queue_size=1).publish(Bool(True))
        self._widget.rightArmRadioButton.setEnabled(False)

    def _on_rightArmRadioButton_pressed(self):
        print("Start rehab - right arm")
        rospy.Publisher('/rehab/right/run', Bool, latch=True, queue_size=1).publish(Bool(True))
        self._widget.leftArmRadioButton.setEnabled(False)
