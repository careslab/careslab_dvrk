        ...
        if not None in self.joint_angles.values():
            ...
            try:
                ...
                if self.__AUTOCAMERA_MODE__ == self.MODE.hardware:
                    ...
                    result = False
                    if self.first_run:
                        #result = self.hw_ecm.move_joint_list(pos, index=[0,1,2,3], interpolate=self.first_run) #reenu (uncomment this line for original 4 joint movement)
                        result = self.hw_ecm.move_joint_list([pos[0],pos[1],pos[2]], index=[0,1,2], interpolate=self.first_run) #reenu (changed this to use 3 joints instead of 4)
                    else:
                        new_zoom = pos[2]
